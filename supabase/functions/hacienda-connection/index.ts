...
Deno.serve(async req => {
  ...
  try {
    ...
    if (['preview', 'xml_preview'].includes(action)) {
      ...
      // En v4.4 Barrio es texto descriptivo (mínimo 5 caracteres), no el código de dos dígitos.
      // OtrasSenas ya contiene la dirección completa, por lo que se omite en este borrador.
      const neighborhood = ''
